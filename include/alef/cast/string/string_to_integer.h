...
std::optional<int32> string_to(const std::string& value, num::radix_flag radix) {
    return __detail::cast::safe_convert_string_to_integer(value, std::stoi, radix);
}
...
#else
    return __detail::cast::safe_convert_string_to_integer(value, std::stoll, radix);
#endif
}
...
#else
    return __detail::cast::safe_convert_string_to_integer(value, std::stoull, radix);
#endif
}
